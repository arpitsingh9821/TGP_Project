...
const CommentSection = ({ applicationId, currentUserId }) => {
  const [commentInput, setCommentInput] = useState("");    // text in main comment box
  const [comments, setComments] = useState([]);            // all comments organized
  const [loading, setLoading] = useState(false);           // main comment submitting
  const [error, setError] = useState("");                  // any error message
  const [editingId, setEditingId] = useState(null);        // which comment is being edited
  const [editInput, setEditInput] = useState("");          // text in edit box
  const [replyingTo, setReplyingTo] = useState(null);      // which comment is being replied to
  const [replyInput, setReplyInput] = useState("");        // text in reply box
  const [submittingReply, setSubmittingReply] = useState(false); // reply submitting

  const api = axios.create({ //makes api calls
  ...
  const startEdit = (c) => {
    setEditingId(c.id); // remember which comment is being edited
    setEditInput(c.comment_text); // pre-fill edit box with current text
  };
