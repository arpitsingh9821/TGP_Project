...
import Cookies from "js-cookie";//Reads user cookie to get name
import { useNavigate } from "react-router-dom";//Navigates to another page programmatically
...
const AppcomDash = () => {
  ...
  const [loading, setLoading] = useState(true);//Shows spinner while data is being fetched
  const [name, setName] = useState(""); // user name
  const [searchBy, setSearchBy] = useState("all");
  const [searchQuery, setSearchQuery] = useState("");
  const navigate = useNavigate();

  //useEffect 1 — Get User's Name
  useEffect(() => {
  ...
  // useEffect 2 — Fetch All Applications
  useEffect(() => {
    axios
