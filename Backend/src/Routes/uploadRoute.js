...
//Multer temporarily saves uploaded files to the uploads/ folder on your server
const fs = require("fs"); //Used to delete files after upload
const path = require("path"); //Helps create correct file paths
const sqlite3 = require("sqlite3").verbose(); //Connects to SQLite database

const router = express.Router();

const db = new sqlite3.Database(path.resolve(__dirname, "../../../database/tgp.db"));

// ✅ Save file with original name to uploads folder
const storage = multer.diskStorage({
  ...
  filename: function (req, file, cb) {
    // Save as timestamp_originalname to avoid conflicts
...
router.post("/upload", authenticate, upload.single("file"), async (req, res) => {
  ...
  // ✅ Create local URL instead of uploading to Cloudinary
  const pdfUrl = `http://localhost:8080/uploads/${file.filename}`;
